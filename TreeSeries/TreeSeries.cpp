...
auto TreeSeries::MirrorPutKV(uint32_t &sid, uint64_t source_id, uint16_t metric_id, uint64_t start_time, uint64_t end_time,
...
auto TreeSeries::RecoveryDiskSlabInfoMirror() -> bool {
    uint32_t size = sizeof(SlabInfo)*ndslab_;
    SlabInfo* disk_slab_info = dstable_;
    auto n = pread(fd_info_, disk_slab_info, size, 0);
    if (n < size) {
...
auto TreeSeries::InsertDB(uint64_t source_id, uint16_t metric_id, uint64_t ts, bool mem, uint32_t sid, uint8_t item_idx) -> bool {
    std::string key;
...
auto TreeSeries::DeleteDB(uint64_t source_id, uint16_t metric_id, uint64_t ts) -> bool {
    std::string key;
    encode_l1key(&key, source_id, metric_id, ts);
...
auto TreeSeries::SearchDB(uint64_t source_id, uint16_t metric_id, uint64_t ts) -> std::pair<SlabInfo*, uint8_t> {
    std::string key;
    encode_l1key(&key, source_id, metric_id, ts);
    std::string val;
...
auto TreeSeries::UpdateDB(uint64_t source_id, uint16_t metric_id, uint64_t ts, bool mem, uint32_t sid, uint8_t item_idx) -> bool {
    std::string key;
    encode_l1key(&key, source_id, metric_id, ts);
    std::string val;
    encode_l1val(&val, mem, sid, item_idx);
...
auto TreeSeries::ScanDB(uint64_t source_id, uint16_t metric_id, uint64_t start_time, uint64_t end_time,
...
auto TreeSeries::TraverseDB() -> void {
    auto iter = db_->NewIterator(read_option_);
