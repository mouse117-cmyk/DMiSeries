...
namespace tsdb::parallel_wal {
    PWalManagementSpan::Wal::Wal(tsdb::head::SpanHead* head, const std::string& dir, int seq, bool tag_log) :
            head_(head),
            dir_(dir),
            seq_(seq),
            cur_log_seq_(0),
...
//        if (boost::filesystem::exists(p) && p.empty()) boost::filesystem::remove_all(dir_);
    ...
    void PWalManagementSpan::Wal::StartBGWork() {
        write_t_.detach();
    }

    void PWalManagementSpan::Wal::AppendSample(tsdbutil::TreeRefSample sample) {
...
//        }
    ...
    void PWalManagementSpan::Wal::AppendSeries(tsdbutil::TreeRefSeries series) {
...
//        }
    ...
    void PWalManagementSpan::Wal::TryExtendLog() {
    ...
    void PWalManagementSpan::Wal::WriteToDiskBG() {
...
//                                auto ms = head_->get_from_forward_index(samples_[i].sgid, samples_[i].mid);
    ...
    void PWalManagementSpan::Wal::Flush() {
        if (tag_log_) {
//            auto cnt = series_queue_.try_dequeue_bulk(series_, ITEM_NUM);
//            if (cnt == 0) {
//                return;
...
//            TryExtendLog();
    ...
    PWalManagementSpan::PWalManagementSpan(tsdb::head::SpanHead* head, const int &wal_num, const std::string &dir) : head_(head), wal_sz_(wal_num) {
    ...
    void PWalManagementSpan::AppendSample(tsdbutil::TreeRefSample sample) {
        sample_wal_[sample.sgid % wal_sz_]->AppendSample(sample);
    }

    void PWalManagementSpan::AppendSeries(tsdbutil::TreeRefSeries series) {
        tag_wal_->AppendSeries(series);
    }

    void PWalManagementSpan::FlushTagLog() {
        tag_wal_->Flush();
    }

    void PWalManagementSpan::FlushSampleLog(int seq) {
        sample_wal_[seq]->Flush();
    }

    void PWalManagementSpan::FlushAll() {
        tag_wal_->Flush();
