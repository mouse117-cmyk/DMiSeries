...
TEST_F(HeadTest, GET_OR_CREATE_TEST) {
  ...
  for (int i = 0; i < metric_num; i++) {
    ...
    for (int j = 0; j < source_num; j++) {
      ...
      auto ms = ret.first;
      std::cout<<"label set: "<<label::lbs_string(ms->labels)<<"\tsgid: "<<ms->series_id_<<"\tmid: "<<ms->metric_id_<<std::endl;
      lset.pop_back();
    }
    lset.clear();
  }
...
TEST_F(HeadTest, FORWARD_INDEX_TEST) {
  ...
  for (int i = 0; i < metric_num; i++) {
    ...
    for (int j = 0; j < source_num; j++) {
      ...
      std::cout<<"label set: "<<label::lbs_string(nms->labels)<<"\tsgid: "<<nms->series_id_<<"\tmid: "<<nms->metric_id_<<std::endl;
      lset.pop_back();
    }
    lset.clear();
  }
  ...
  auto process_labels = [&] (int metric_start, int sg_start) {
    ...
    for (int i = metric_start; i < metric_start+metric_num; i++) {
      ...
      for (int j = sg_start; j < sg_start+source_num; j++) {
        ...
        hd->set_to_forward_index(j, i,  ms);
//        auto nms = hd->get_from_forward_index(j, i);
//        std::cout<<"label set: "<<label::lbs_string(nms->labels)<<"\tid: "<<nms->ref<<"\tsgid: "<<nms->series_id_<<"\tmid: "<<nms->metric_id_<<std::endl;
        lset.pop_back();
      }
      lset.clear();
    }
...
//        std::cout<<"j: "<<j<<"\tnewly created: "<<ret.second<<std::endl;
        auto ms = ret.first;
//        std::cout<<"label set: "<<label::lbs_string(ms->labels)<<"\tid: "<<ms->ref<<"\tsgid: "<<ms->series_id_<<"\tmid: "<<ms->metric_id_<<std::endl;
        if (ret.second) {
//          std::cout<<"label set: "<<label::lbs_string(ms->labels)<<"\tid: "<<ms->ref<<"\tsgid: "<<ms->series_id_<<"\tmid: "<<ms->metric_id_<<std::endl;
        }
