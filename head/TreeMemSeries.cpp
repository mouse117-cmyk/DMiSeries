...
leveldb::Status TreeMemSeries::_flush_mirror(slab::ValueLog *value_log, int64_t txn) {
    ...
    key_.clear();
    value_log->EnCodeKey(&key_,series_id_,metric_id_,tuple_st_);
    ...
    leveldb::EncodeFixed64BE(
            reinterpret_cast<char*>(&chunk_.bstream.stream[0] + 3), series_id_);
    leveldb::Status s;
    ...
    chunk_num_++;
    while(!value_log->MirrorPutKV(sid_, series_id_,tuple_st_,
...
//        while(!value_log->GetMemSlabID(sid_, series_id_, metric_id_)){
//        }
//    }
  ...
  chunk_num_++;
  while(!value_log->ConPutKV(sid_, series_id_, metric_id_,tuple_st_,max_time_,(uint8_t *)(&chunk_.bstream.stream[0]), slab::CHUNK_SIZE, txn));
//  while(!flag){
//      value_log->GetMemSlabID(sid_, series_id_, metric_id_);
//      flag = value_log->ConPutKV(sid_, series_id_, metric_id_,tuple_st_,max_time_,(uint8_t *)(&chunk_.bstream.stream[0]), slab::CHUNK_SIZE, txn);
//  }
...
bool TreeMemSeries::append(slab::ValueLog* value_log, int64_t timestamp, double value,
    ...
    if (timestamp > global_max_time_) global_max_time_ = timestamp;
    if (min_time_ == INT64_MAX) min_time_ = timestamp;
    int64_t time_boundary = timestamp / leveldb::PARTITION_LENGTH * leveldb::PARTITION_LENGTH;
