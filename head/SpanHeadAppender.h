#ifndef TEMPUSCSTORE_SPANHEADAPPENDER_H
#define TEMPUSCSTORE_SPANHEADAPPENDER_H
#pragma once
...
//             }
            virtual leveldb::Status add(const label::Labels& lset, int64_t t,
                ...
                sgid = s.first->series_id_;
                mid = s.first->metric_id_;
                if (s.second) series.emplace_back(s.first->series_id_,s.first->metric_id_, lset, s.first);

                return add_fast(sgid, t, v);
            }

          virtual leveldb::Status add(const label::Labels& lset,
                                                 int64_t t, double v,uint64_t &series_id,
                                                 uint64_t epoch = 0) override {
              if(lset.size()==0){
                LOG_DEBUG<<"lset size = 0\n";
              }
              std::pair<TreeMemSeries*, bool> s = ts_head_->get_or_create_by_thmap(lset, epoch);
              series_id = s.first->series_id_;
              if (s.second) series.emplace_back(s.first->series_id_,s.first->metric_id_, lset, s.first);

              return add_fast(series_id, t, v);
            }
            ...
            //mock
            virtual leveldb::Status add_fast(uint64_t series_id, int64_t t, double v) override {
              int64_t txn = 0;
              auto ms = ts_head_->read_flat_forward_index(series_id);
              if(ms == nullptr){
                return leveldb::Status::IOError();
              }
              samples.emplace_back(series_id, 1, t, v, txn);
              return leveldb::Status::OK();
            }
...
#endif //TEMPUSCSTORE_SPANHEADAPPENDER_H
