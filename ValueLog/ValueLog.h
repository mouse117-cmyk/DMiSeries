#ifndef TSDB_UNITTEST_VALUELOG_H
#define TSDB_UNITTEST_VALUELOG_H
#include <assert.h>
#include <fcntl.h>
#include <sys/ioctl.h>
#include <sys/mman.h>
...
static const int CX_NUM = 1;
static const int CY_NUM = 1000000;
...
//  auto DestroyCtable() -> void;
  ...
  inline auto GetMemSlabInfo(uint64_t series_id,uint16_t metric_id,uint32_t sid,uint8_t &idx) -> SlabInfo* {
      for(uint8_t i=0;i<mstable_[sid].offset_;i++){
          if(mstable_[sid].series_id_[i] == series_id){
              idx = i;
          }
      }
      return &mstable_[sid];
  ...
  inline auto FreeSlab(SlabInfo* sinfo) -> void{
    ...
    for(uint8_t i=0;i<sinfo->offset_;i++){
        sinfo->series_id_[i] = 0;
  ...
  inline auto GetSinfoSourceID(SlabInfo * sinfo,uint8_t idx) -> uint64_t {
      ...
      return uint64_t(uint64_t((uint64_t)(sinfo->series_id_[idx])*(uint64_t)(CY_NUM))+uint64_t(cinfo->class_series_id_));
  }
...
void EnCodeKey(std::string* str, uint64_t series_id, uint16_t metric_id, uint64_t t) {
//    leveldb::PutFixed64BE(str, series_id);
//    leveldb::PutFixed16(str,metric_id);
//    leveldb::PutFixed64BE(str, t);

    encode_l1key(str, series_id, metric_id, t);
}
void DeCodeKey(std::string str, uint64_t& series_id, uint16_t& metric_id, uint64_t& t) {
//    series_id = leveldb::DecodeFixed64BE(&str.c_str()[0]);
//    metric_id = leveldb::DecodeFixed16(&str.c_str()[8]);
//    t = leveldb::DecodeFixed64BE(&str.c_str()[10]);

    decode_l1key(str, series_id, metric_id, t);
}

  auto GetMemSlabID(uint32_t &sid, uint64_t series_id, uint16_t metric_id)->bool;
  auto GetDiskSlabID(uint32_t &sid, uint64_t series_id, uint16_t metric_id)->bool;

  auto GetMemSlabID(uint32_t &sid, uint64_t series_id, uint16_t metric_id,uint64_t start_time)->bool;
  auto GetDiskSlabID(uint32_t &sid, uint64_t series_id, uint16_t metric_id, uint64_t start_time)->bool;
  ...
  auto PutKV(uint32_t& sid,uint64_t series_id, uint16_t metric_id, uint64_t key, uint64_t end_time, const uint8_t* value,uint32_t value_len=CHUNK_SIZE, uint64_t txn = 0)->bool;
  auto ConPutKV(uint32_t& sid,uint64_t series_id, uint16_t metric_id, uint64_t key, uint64_t end_time, const uint8_t* value,uint32_t value_len=CHUNK_SIZE, uint64_t txn = 0)->bool;

  auto MirrorPutKV(uint32_t& sid, uint64_t series_id, uint64_t key, uint64_t end_time, const uint8_t* value,uint32_t value_len=CHUNK_SIZE, uint64_t txn = 0)->bool;
  ...
  auto RecoveryDiskSlabInfoMirror() -> bool;
  ...
  auto InsertMT(uint64_t series_id, uint16_t metric_id, uint64_t ts, SlabInfo* sinfo) -> bool;
  auto RemoveMT(uint64_t series_id, uint16_t metric_id, uint64_t ts) -> bool ;
  auto BatchRemoveMT(std::vector<SlabInfo*> sinfo_arr) -> bool;
  auto BatchInsertMT(std::vector<SlabInfo*> sinfo_arr) -> bool;
  auto SearchMT(uint64_t series_id, uint16_t metric_id, uint64_t ts) -> SlabInfo*;
  auto UpdateMT(uint64_t series_id, uint16_t metric_id, uint64_t ts, SlabInfo* sinfo) -> bool;
  auto ScanMT(uint64_t series_id, uint16_t metric_id, uint64_t start_time, uint64_t end_time,std::vector<const SlabInfo*>&sinfo_arr)->void;

  auto InsertDB(uint64_t series_id, uint64_t ts, bool mem, uint32_t sid, uint32_t segment_id, uint8_t item_idx) -> bool;
  auto DeleteDB(uint64_t series_id, uint64_t ts) -> bool;
  auto SearchDB(uint64_t series_id, uint64_t ts) -> std::pair<SlabInfo*, uint8_t>;
  auto UpdateDB(uint64_t series_id, uint64_t ts, bool mem, uint32_t sid, uint32_t segment_id, uint8_t item_idx) -> bool;
  auto ScanDB(uint64_t series_id, uint64_t start_time, uint64_t end_time, std::vector<const SlabInfo*>&sinfo_arr) -> void;
  auto TraverseDB() -> void;

  auto InsertMTAsyn(uint64_t series_id, uint16_t metric_id, uint64_t ts, SlabInfo* sinfo) -> bool;
  auto RemoveMTAsyn(uint64_t series_id, uint16_t metric_id, uint64_t ts) -> bool ;
  auto UpdateMTAsyn(uint64_t series_id, uint16_t metric_id, uint64_t ts, SlabInfo* sinfo) -> bool;
  auto ScanMTAsyn(uint64_t series_id, uint16_t metric_id, uint64_t start_time, uint64_t end_time) -> std::vector<const SlabInfo*>;
  auto Scan(uint64_t series_id, uint64_t start_time, uint64_t end_time,
            std::vector<std::pair<const SlabInfo*, Slab*>>& slab_arr) -> bool;
  ...
  void SetDB(leveldb::DB* db) {db_ = db;}
...
#endif  // TSDB_UNITTEST_VALUELOG_H
