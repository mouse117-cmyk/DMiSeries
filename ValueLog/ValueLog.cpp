...
auto ValueLog::GetMemSlabID(uint32_t &sid, uint64_t series_id_, uint16_t metric_id) -> bool {
  SlabInfo* sinfo;
  SlabClass* cinfo = &ctable[metric_id%CX_NUM][series_id_%CY_NUM];
 // std::cout<<"***\n";
  cinfo->mtx_.lock();
//  std::cout<<"nmfree: "<<nfree_msinfoq_<<" nmevict:"<<n_mem_evict_sinfo_<<std::endl;
  //std::cout<<"*\n";
  if(cinfo->slab_id_ == UINT32_MAX||mstable_[cinfo->slab_id_].offset_
    == MAX_SERIES_NUM|| SlabFull(&mstable_[cinfo->slab_id_])){
      ...
      if(!flag){
          ...
          for (uint8_t j = 0; j < tmp_msinfo->offset_
            ; j++) {
...
//              UpdateMT(GetSinfoSourceID(tmp_msinfo,j), GetSinfoMetricID(tmp_msinfo,j),tmp_msinfo->end_time_[j], tmp_dsinfo);
              UpdateDB(GetSinfoSourceID(tmp_msinfo,j),tmp_msinfo->end_time_[j], false, tmp_dsinfo->slab_id_, tmp_dsinfo->segment_id_, 0);
          }
//          FreeSlab(sinfo);
//          sid = sinfo->slab_id_;
//          sinfo->free_ = false;
//          sinfo->series_id__[0] = (uint8_t)(series_id_ / CY_NUM);
...
//          cinfo->slab_id_ = sinfo->slab_id_;
          ...
          tmp_msinfo->series_id_[0] = (uint8_t)(series_id_ / CY_NUM);
          tmp_msinfo->offset_
        = 1;
          cinfo->slab_id_ = tmp_msinfo->slab_id_;
          tmp_msinfo->cid_x_ = metric_id % CX_NUM;
          tmp_msinfo->cid_y_ = series_id_ % CY_NUM;
      }else{
          sid = sinfo->slab_id_;
          nused_msinfoq_.fetch_add(1);
          nfree_msinfoq_.fetch_sub(1);
          sinfo->free_ = false;
          sinfo->series_id_[0] = (uint8_t)(series_id_ / CY_NUM);
          sinfo->offset_
        = 1;
          cinfo->slab_id_ = sinfo->slab_id_;
          sinfo->cid_x_ = metric_id % CX_NUM;
          sinfo->cid_y_ = series_id_ % CY_NUM;
      }
      cinfo->mtx_.unlock();
      return flag;
  }
  ...
  sid = sinfo->slab_id_;
  sinfo->series_id_[sinfo->offset_
    ] = (uint8_t)(series_id_ / CY_NUM);
  sinfo->offset_
  ++;
  cinfo->mtx_.unlock();
  return true;
...
auto ValueLog::GetDiskSlabID(uint32_t &sid, uint64_t series_id_, uint16_t metric_id) -> bool {
  SlabInfo* sinfo;
  ...
  sinfo->free_ = false;
  sinfo->series_id_[0] = series_id_;

  return flag;
}

auto ValueLog::GetMemSlabID(uint32_t &sid, uint64_t series_id_, uint16_t metric_id, uint64_t start_time) -> bool {
    SlabInfo* sinfo;
    SlabClass* cinfo = &ctable[metric_id%CX_NUM][series_id_%CY_NUM];
    std::cout<<"**********\n";
    cinfo->mtx_.lock();
    ...
    if(cinfo->slab_id_ == UINT32_MAX||mstable_[cinfo->slab_id_].offset_
      == MAX_SERIES_NUM || SlabFull(&mstable_[cinfo->slab_id_])){
        ...
        if(!flag){
            ...
            n_mem_evict_sinfo_.fetch_sub(1);
            nused_msinfoq_.fetch_add(1);
            for (uint8_t j = 0; j < sinfo->offset_
              ; j++) {
...
//                UpdateMT(GetSinfoSourceID(sinfo,j), GetSinfoSourceID(sinfo,j),sinfo->end_time_[j], dsinfo);
                UpdateDB(GetSinfoSourceID(sinfo,j),sinfo->end_time_[j], false, dsinfo->slab_id_, dsinfo->segment_id_,0);
            }
            FreeSlab(sinfo);
            sid = sinfo->slab_id_;
            sinfo->free_ = false;
            sinfo->series_id_[0] = series_id_ / CY_NUM;
            sinfo->offset_
          = 1;
            cinfo->slab_id_ = sinfo->slab_id_;
    ...
    sinfo->start_time_[sinfo->offset_
      ] = start_time;
    sinfo->offset_
  ++;
    cinfo->mtx_.unlock();
...
auto ValueLog::GetDiskSlabID(uint32_t &sid, uint64_t series_id_, uint16_t metric_id, uint64_t start_time) -> bool {
  SlabInfo* sinfo;
    auto flag = free_dsinfoq_->try_pop(sinfo);
  if(!flag){
    return flag;
...
auto ValueLog::MirrorPutKV(uint32_t &sid, uint64_t series_id_, uint64_t start_time, uint64_t end_time,
                          const uint8_t *value, uint32_t value_len, uint64_t txn) -> bool {
    SlabClass* cinfo = &ctable[0][series_id_%CY_NUM];
    cinfo->mtx_.lock();
...
//                UpdateMT(GetSinfoSourceID(tmp_msinfo,j), GetSinfoMetricID(tmp_msinfo,j),tmp_msinfo->end_time_[j], tmp_dsinfo);
    ...
    sinfo->cid_y_ = series_id_ % CY_NUM;
    sinfo->cid_x_ = 0;
    uint8_t f_sgid = series_id_ / CY_NUM;
    uint8_t f_mid = 0;
    ...
    bool is_exist = false;
    uint8_t idx = 0;
    for(uint8_t i=0;i<sinfo->offset_
      ;i++){
        if(sinfo->series_id_[i] == f_sgid){
            is_exist = true;
            idx = i;
            break;
        }
    ...
    if(is_exist){
//        RemoveMT(series_id_,metric_id,sinfo->end_time_[idx]);
        DeleteDB(series_id_, sinfo->end_time_[idx]);
        sinfo->end_time_[idx] = end_time;
//        InsertMT(GetSinfoSourceID(sinfo,idx), GetSinfoMetricID(sinfo,idx),sinfo->end_time_[idx],sinfo);
        InsertDB(GetSinfoSourceID(sinfo, idx), sinfo->end_time_[idx], sinfo->mem_, sinfo->slab_id_, sinfo->segment_id_, sinfo->nalloc_-1);
    }else{
        idx = sinfo->offset_
      ;
        sinfo->offset_
      ++;
        sinfo->series_id_[idx] = f_sgid;
        assert(GetSinfoSourceID(sinfo,idx)==series_id_);
...
auto ValueLog::ConPutKV(uint32_t &sid, uint64_t series_id_, uint16_t metric_id, uint64_t start_time, uint64_t end_time,
                          const uint8_t *value, uint32_t value_len, uint64_t txn) -> bool {
    SlabClass* cinfo = &ctable[metric_id%CX_NUM][series_id_%CY_NUM];
    cinfo->mtx_.lock();
    SlabInfo* sinfo = nullptr;
...
auto ValueLog::PutKV(uint32_t& sid, uint64_t series_id_, uint16_t metric_id, uint64_t key, uint64_t end_time,
                           const uint8_t *value, uint32_t value_len, uint64_t txn) -> bool {
  if (!ValidSid(sid)) {
    return false;
  }
...
auto ValueLog::BatchWrite() -> bool {
    ...
    for(uint32_t i=0;i<count;i++){
        ...
        for(uint8_t i=0;i<temp_msinfo->offset_
          ;i++){
...
//            UpdateMT(GetSinfoSourceID(temp_msinfo,i), GetSinfoMetricID(temp_dsinfo,i),temp_msinfo->end_time_[i],temp_dsinfo);
            UpdateDB(GetSinfoSourceID(temp_msinfo,i),temp_msinfo->end_time_[i],false,temp_dsinfo->slab_id_, temp_dsinfo->segment_id_,0);
        }
...
auto ValueLog::OptBatchWrite() -> bool {
    if(nfull_msinfoq_.load() < setting_.write_batch_size_){
        return false;
    }
    std::vector<std::future<bool>> future_results;
    ...
    for (uint32_t i = 0; i < count; i++) {
        auto msinfo = full_msinfo_wqueue[i];
        uint32_t dsid = (msinfo->segment_id_*segment_size_ + msinfo->slab_id_) % ndslab_;
        msinfo->segment_id_++;
        global_segment_id_ = std::max(global_segment_id_, msinfo->segment_id_);
        free_dsinfo_wqueue[i] = &dstable_[dsid];
    }
    ...
    for(uint32_t i=0;i<count;i++){
        ...
        for(uint8_t i=0;i<temp_msinfo->offset_;i++){
            UpdateDB(GetSinfoSourceID(temp_msinfo,i),temp_msinfo->end_time_[i],false,temp_dsinfo->slab_id_, temp_dsinfo->segment_id_,0);
        }
        FreeSlab(temp_msinfo);
    }
    ...
    auto ValueLog::OptBatchWriteLazyFlush() -> bool {
        ...
        for (; fi < count; fi++) {
            ...
            for(uint8_t j=0;j<free_dsinfo_wqueue[fi]->offset_
              ;j++){
                DeleteDB(GetSinfoSourceID(free_dsinfo_wqueue[fi],i),free_dsinfo_wqueue[fi]->end_time_[i]);
            }
...
auto ValueLog::RecoveryDiskSlabInfoMirror() -> bool {
    ...
    for(uint32_t i=0;i<ndslab_;i++){
        ...
        if(!sinfo->free_){
            for(uint8_t j=0;j<sinfo->offset_
              ;j++){
                InsertDB(GetSinfoSourceID(sinfo,j), sinfo->end_time_[j],sinfo->mem_,sinfo->slab_id_, sinfo->segment_id_,0);
            }
        }
    }
    return true;
...
auto ValueLog::MigrateRead(std::vector<std::pair<SlabInfo *, Slab *>>&migrate_slab_array, bool& is_seq) -> bool {
    ...
    for(uint32_t i=0;i<count;i++){
        for(uint8_t j=0;j<used_dsinfo_queue[i]->offset_
          ;j++){
...
//            RemoveMT(GetSinfoSourceID(used_dsinfo_queue[i],j), GetSinfoMetricID(used_dsinfo_queue[i],j),used_dsinfo_queue[i]->end_time_[j]);
            DeleteDB(GetSinfoSourceID(used_dsinfo_queue[i],j),used_dsinfo_queue[i]->end_time_[j]);
        }
    }
    for (uint32_t i = 0; i < count; i++) {
...
auto ValueLog::InsertMT(uint64_t series_id_, uint16_t metric_id, uint64_t ts, SlabInfo* sinfo) -> bool {
  std::string packed_key;
  EnCodeKey(&packed_key,series_id_,metric_id,ts);
  return mass_tree_.insert_value(packed_key.c_str(), packed_key.size(), sinfo);
}

auto ValueLog::RemoveMT(uint64_t series_id_, uint16_t metric_id, uint64_t ts) -> bool {
  std::string packed_key;
  EnCodeKey(&packed_key,series_id_,metric_id,ts);
  return mass_tree_.remove_value(packed_key.c_str(), packed_key.size());
}

auto ValueLog::SearchMT(uint64_t series_id_, uint16_t metric_id, uint64_t ts) -> SlabInfo* {
  std::string packed_key;
  EnCodeKey(&packed_key,series_id_,metric_id,ts);
  return mass_tree_.get_value(packed_key.c_str(), packed_key.size());
}

auto ValueLog::UpdateMT(uint64_t series_id_, uint16_t metric_id, uint64_t ts, SlabInfo* sinfo) -> bool {
  std::string packed_key;
  EnCodeKey(&packed_key,series_id_,metric_id,ts);
  return mass_tree_.update_value(packed_key.c_str(), packed_key.size(), sinfo, false);
}

auto ValueLog::ScanMT(uint64_t series_id_, uint16_t metric_id, uint64_t start_time, uint64_t end_time, std::vector<const SlabInfo*>&sinfo_arr) -> void {
  std::string start_key, end_key;
  EnCodeKey(&start_key, series_id_, metric_id, start_time);
  EnCodeKey(&end_key, series_id_, metric_id, end_time);
...
auto ValueLog::InsertDB(uint64_t series_id_, uint64_t ts, bool mem, uint32_t sid, uint32_t segment_id, uint8_t item_idx) -> bool {
    std::string key;
    encode_l1key(&key, series_id_, 1, ts);
    std::string val;
    encode_l1val(&val, mem, sid, segment_id, item_idx);
    auto status = db_->Put(write_option_, key, val);
    // bool ret=kv_server->putValue(key.data(),key.size(),val.data(),val.size());
    return status.ok();
    // return ret;
}

auto ValueLog::DeleteDB(uint64_t series_id_, uint64_t ts) -> bool {
    std::string key;
    encode_l1key(&key, series_id_, 1, ts);
    auto status = db_->Delete(write_option_, key);
    // bool ret=kv_server->delValue(key.data(),key.size());
    return status.ok();
    // return ret;
}

auto ValueLog::SearchDB(uint64_t series_id_, uint64_t ts) -> std::pair<SlabInfo*, uint8_t> {
    std::string key;
    encode_l1key(&key, series_id_, 1, ts);
    std::string val;
    ...
    uint32_t segment_id;
    uint8_t item_idx;
    decode_l1val(val, mem, sid, segment_id, item_idx);
    SlabInfo* sinfo = nullptr;
    if (mem) {
        sinfo = &mstable_[sid];
    } else {
...
auto ValueLog::UpdateDB(uint64_t series_id_, uint64_t ts, bool mem, uint32_t sid, uint32_t segment_id, uint8_t item_idx) -> bool {
    std::string key;
    encode_l1key(&key, series_id_, 1, ts);
    std::string val;
    encode_l1val(&val, mem, sid, segment_id, item_idx);
...
auto ValueLog::ScanDB(uint64_t series_id_, uint64_t start_time, uint64_t end_time,
                        std::vector<const SlabInfo *> &sinfo_arr) -> void {
    auto iter = db_->NewIterator(read_option_);
    std::string key;
    encode_l1key(&key, series_id_, 1, start_time);
...
auto ValueLog::InsertMTAsyn(uint64_t series_id_, uint16_t metric_id, uint64_t ts, SlabInfo* sinfo) -> bool {
  this->pool_.enqueue([this, series_id_,metric_id, ts,sinfo]{
    this->InsertMT( series_id_,metric_id, ts,sinfo);
  });
  return true;
}

auto ValueLog::RemoveMTAsyn(uint64_t series_id_, uint16_t metric_id, uint64_t ts) -> bool {
  this->pool_.enqueue([this, series_id_,metric_id, ts]{
    this->RemoveMT( series_id_,metric_id, ts);
  });
  return true;
}

auto ValueLog::UpdateMTAsyn(uint64_t series_id_, uint16_t metric_id, uint64_t ts, SlabInfo* sinfo) -> bool {
  this->pool_.enqueue([this, series_id_,metric_id, ts,sinfo]{
    this->UpdateMT( series_id_,metric_id, ts,sinfo);
  });
  return true;
}

auto ValueLog::ScanMTAsyn(uint64_t series_id_, uint16_t metric_id, uint64_t start_time, uint64_t end_time) -> std::vector<const SlabInfo*> {
  std::vector<const SlabInfo*> sinfo_arr;
  this->pool_.enqueue([this, series_id_,metric_id, start_time,end_time, &sinfo_arr]{
   this->ScanMT( series_id_,metric_id, start_time,end_time,sinfo_arr);
  });
  return sinfo_arr;
}

auto ValueLog::Scan(uint64_t series_id_, uint64_t start_time, uint64_t end_time,
                    std::vector<std::pair<const SlabInfo*, Slab*>>& slab_arr)
    -> bool {
  std::vector<const SlabInfo*> sinfo_arr;
//    ScanMT(series_id_,metric_id,start_time,end_time,sinfo_arr);
  ScanDB(series_id_, start_time, end_time, sinfo_arr);
  ...
  for(auto &it:sinfo_arr){
    ...
    //   }));
    // }
      if(it->segment_id_ == GetMemSlabInfo(it->slab_id_)->segment_id_){
          future_slab_arr.emplace_back(GetMemSlab(it->slab_id_));
      }else{
          future_slab_arr.emplace_back(pool_.enqueue([this,it]{
            return this->ReadDiskSlab(it);
...
auto ValueLog::BatchRemoveMT(std::vector<SlabInfo*> sinfo_arr) -> bool {
  for (auto &it : sinfo_arr) {
    for(uint8_t i=0;i<it->offset_
      ;i++){
     //   RemoveMTAsyn(it->series_id__[i], it->metric_id_[i], it->start_time_[i]);
...
auto ValueLog::BatchInsertMT(std::vector<SlabInfo*> sinfo_arr) -> bool {
  for (auto &it : sinfo_arr) {
      for(uint8_t i=0;i<it->offset_
        ;i++){
      //    InsertMTAsyn(it->series_id__[i], it->metric_id_[i], it->start_time_[i],it);
...
void encode_l1val(std::string *val, bool mem, uint32_t sid, uint32_t segment_id, uint8_t item_idx) {
    val->push_back(mem);
    leveldb::PutFixed32BE(val, sid);
    leveldb::PutFixed32BE(val, segment_id);
    val->push_back(item_idx);
}

void decode_l1val(std::string val, bool& mem, uint32_t &sid, uint32_t& segment_id, uint8_t &item_idx) {
    mem = val[0];
    sid = leveldb::DecodeFixed32BE(val.data()+1);
    segment_id = leveldb::DecodeFixed32BE(val.data()+5);
    item_idx = val[9];
}
