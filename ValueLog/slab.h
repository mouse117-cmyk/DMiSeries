...
struct SlabInfo {
  ...
  uint8_t series_id_[MAX_SERIES_NUM];
  ...
  uint8_t offset_;
//  SlabInfo() = default;
  SlabInfo() : cid_x_(0), cid_y_(0), slab_id_(0), segment_id_(std::numeric_limits<uint32_t>::max()), nalloc_(0), mem_(true), free_(true), offset_(0) {}
//  SlabInfo(uint64_t sgid, uint16_t mid, uint16_t nalloc = SLAB_SIZE / SLAB_ITEM_SIZE) : series_id_(sgid), metric_id_(mid), nalloc_(nalloc){};
};
