...
//        tree_series_->PrintItem(item);
    ...
    SpanQuerierSeries::SpanQuerierSeries(const SpanQuerier *q, uint64_t sgid, uint16_t mid) : q_(q), sgid_(sgid), mid_(mid), init_(false) {
        auto tms = q_->head_->read_flat_forward_index(sgid, mid);
//        if (tms == nullptr) {
//            tms = q_->head_->get_from_forward_index(sgid-1, mid);
//        }
    ...
    const label::Labels& SpanQuerierSeries::labels() const {
        ...
        q_->head_->series(sgid_, mid_, lset_, &head_chunk_contents_);
        init_ = true;
        return lset_;
    }
...
//            }
    ...
    SpanQuerierSeriesSet::SpanQuerierSeriesSet(const tsdb::querier::SpanQuerier *q, const std::vector<std::shared_ptr<label::MatcherInterface>> &l)
        ...
        for (auto& it : matchers_) {
            if (it->name() == "__name__") {
                bool is_exist = true;
                mid_ = q_->head_->get_or_create_metric_id(label::Labels({label::Label(it->name(), it->value())}), is_exist);
                if (!is_exist) mid_ = 0;
                break;
            }
        }
...
//        if (q_->head_->get_from_forward_index(sgid_, mid_) == nullptr) return nullptr;
        if (q_->head_->read_flat_forward_index(sgid_, mid_) == nullptr) return nullptr;
        return std::unique_ptr<querier::SeriesInterface>(new SpanQuerierSeries(q_, sgid_, mid_));
    }
