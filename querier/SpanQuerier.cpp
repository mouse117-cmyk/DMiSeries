...
namespace tsdb::querier {
    ...
    std::pair<std::vector<int64_t>, std::vector<double>> DecodeChunk(const std::string& chunk_content, uint64_t &series_id, uint16_t &mid) {
        ...
        series_id = leveldb::DecodeFixed64BE(s.data() + 3);
    ...
    SpanHeadIterator::SpanHeadIterator(const std::string& chunk_content, int64_t mint, int64_t maxt)
            : end_(false),
              min_time_(mint),
              max_time_(maxt),
              init_(false){
        auto s = leveldb::Slice(chunk_content.data(), chunk_content.size());
        leveldb::Slice tmp_value;
        auto m = leveldb::DecodeFixed16(s.data() + 1);
        series_id_ = leveldb::DecodeFixed64BE(s.data() + 3);
        tmp_value = leveldb::Slice(s.data() + 11, s.size() - 11);
        chunk_ = tsdb::chunk::XORChunk(reinterpret_cast<const uint8_t*>(tmp_value.data()), tmp_value.size());
        iter_ = chunk_.xor_iterator();
    }

    bool SpanHeadIterator::seek(int64_t t) const {
    ...
    std::pair<int64_t, double> SpanHeadIterator::at() const {
        if (end_) return {0, 0};
        return iter_->at();
    }

    bool SpanHeadIterator::next() const {
        if (!init_) {
            init_ = true;
            while (true) {
                if (!iter_->next()) {
    ...
    SlabArrayIterator::SlabArrayIterator(slab::ValueLog *value_log, std::vector<std::pair<const slab::SlabInfo*, slab::Slab*>> slab_array, int64_t min_time, int64_t max_time, uint64_t series_id, leveldb::Cache *cache)
    ...
    void SlabArrayIterator::decode_value(const leveldb::Slice& key, const leveldb::Slice& s) const {
        ...
        uint64_t series_id = leveldb::DecodeFixed64BE(s.data() + 3);

        if (series_id != series_id_) return;
...
//                        item_idx_ = 0;
                        ...
                        auto item = GetSlabItem(slab_iter_[slab_idx_].first, slab_iter_[slab_idx_].second, item_idx_);
                        DecodeItemChunkHeader(item, mid, series_id, start_time);
                        item_idx_++;
                        if (series_id != series_id_) continue;
                        DecodeItemChunk(item);
                    }
                }
            }
...
//                    item_idx_ = 0;
    ...
    int SlabArrayIterator::GetItemIdx(const slab::SlabInfo *sinfo, slab::Slab *s) const {
        ...
        for (int i = 0; i < sinfo->nalloc_; i++) {
            ...
            int64_t start_time = std::numeric_limits<int64_t>::max();
            DecodeItemChunkHeader(item, mid, series_id, start_time);
//            std::cout<<mid<<" "<<series_id<<" "<<start_time<<std::endl;
            if (series_id != series_id_) continue;
...
//        value_log_->PrintItem(item);
    ...
    void SlabArrayIterator::DecodeItemChunk(slab::Item* item) const {
        ...
        std::string key;
        value_log_->EnCodeKey(&key, series_id_, 1, item->timestamp_);
        decode_value(key, leveldb::Slice(reinterpret_cast<const char *>(item->chunk_), slab::CHUNK_SIZE));
    }

    void SlabArrayIterator::DecodeItemChunkHeader(slab::Item *item, uint16_t &mid, uint64_t &series_id, int64_t &start_time) const {
    ...
    ValueLogIterator::ValueLogIterator(slab::ValueLog *value_log, int64_t min_time, int64_t max_time, uint64_t series_id, leveldb::Cache *cache)
            : min_time_(min_time),
              max_time_(max_time),
              series_id_(series_id),
              value_log_(value_log),
              slab_array_(){
        if (min_time_ < 0) min_time_ = 0;
        value_log->Scan(series_id, min_time, max_time, slab_array_);
        slab_iter_ = std::unique_ptr<SlabArrayIterator>(new SlabArrayIterator(value_log, slab_array_, min_time, max_time, series_id, cache));
    }
    ...
    MemtableIterator::MemtableIterator(slab::ValueLog *value_log, leveldb::MemTable* mem, int64_t min_time, int64_t max_time, uint64_t series_id, leveldb::Cache *cache)
            : min_time_(min_time),
              max_time_(max_time),
              series_id_(series_id),
              value_log_(value_log),
        ...
        GetSlabArray();
        slab_iter_ = std::make_unique<SlabArrayIterator>(value_log, slab_array_, min_time_, max_time_, series_id, cache);
    }
    ...
    void MemtableIterator::GetSlabArray() {
        std::string key;
        value_log_->EnCodeKey(&key, series_id_, 1, min_time_);
        ...
        uint64_t series_id;
        uint64_t t;
        value_log_->DeCodeKey(mem_iter_->key().ToString(), series_id, mid, t);
        if (series_id != series_id_) {
            mem_iter_->Next();
            if (!mem_iter_->Valid()) {
    ...
    L0ValueLogIterator::L0ValueLogIterator(const querier::SpanQuerier *q, uint64_t series_id, leveldb::Iterator *it, int64_t mint, int64_t maxt)
            : q_(q),
              ...
              iter_(it) {
        GetSlabArray();
        slab_iter_ = std::make_unique<SlabArrayIterator>(value_log_, slab_array_, min_time_, max_time_, series_id);
    }

    L0ValueLogIterator::L0ValueLogIterator(const querier::SpanQuerier *q, int partition, uint64_t series_id, int64_t mint, int64_t maxt)
            : q_(q),
              value_log_(q->value_log_),
        ...
        q_->current_->AddIterators(leveldb::ReadOptions(), 0, q_->l0_indexes_[partition_], &list, series_id_, 1);
    ...
    void L0ValueLogIterator::GetSlabArray() {
        ...
        uint64_t series_id;
        uint64_t t;
        value_log_->DeCodeKey(iter_->key().ToString(), series_id, mid, t);
        if (series_id != series_id_) {
            iter_->Next();
            if (!iter_->Valid()) {
                err_.set("error next seek 2");
    ...
    L1ValueLogIterator::L1ValueLogIterator(const querier::SpanQuerier *q, int partition, uint64_t series_id, int64_t mint, int64_t maxt)
            : q_(q),
              value_log_(q->value_log_),
              partition_(partition),
              series_id_(series_id),
              ...
              max_time_(maxt) {
        std::vector<leveldb::Iterator*> list;
        q_->current_->AddIterators(leveldb::ReadOptions(), 1, q_->l1_indexes_[partition_], &list, series_id_, 1);
        iter_.reset(leveldb::NewMergingIterator(q_->cmp_, &list[0], list.size()));
    ...
    SpanQuerierSeries::SpanQuerierSeries(const SpanQuerier *q, uint64_t series_id) : q_(q), series_id_(series_id), init_(false) {
        auto tms = q_->head_->read_flat_forward_index(series_id);
//        if (tms == nullptr) {
//            tms = q_->head_->get_from_forward_index(series_id-1, mid);
//        }
        head_flush_time_ = tms->head_flush_time();
        level_flush_time_ = tms->level_flush_time();
    }

    const label::Labels& SpanQuerierSeries::labels() const {
        if (init_) return lset_;
        q_->head_->series(series_id_, lset_, &head_chunk_contents_);
        init_ = true;
        return lset_;
    }

    std::unique_ptr<querier::SeriesIteratorInterface> SpanQuerierSeries::iterator() {
        if (!init_) {
            q_->head_->series(series_id_, lset_, &head_chunk_contents_);
            init_ = true;
        }
        ...
        if (q_->min_time_ <= level_flush_time_) {
            ...
            for (int i = 0; i < q_->l1_indexes_.size(); i++) {
                iters.push_back(new L1ValueLogIterator(q_, i, series_id_, q_->min_time_, q_->max_time_));
            }
            ...
            for (int i = 0; i < q_->l0_indexes_.size(); i++) {
                ...
                std::vector<leveldb::Iterator*> list;
                q_->current_->AddIterators(leveldb::ReadOptions(), 0, q_->l0_indexes_[i], &list, series_id_, 1);
                for (leveldb::Iterator* it : list) {
                    mit->push_back(new L0ValueLogIterator(q_, series_id_, it, q_->min_time_, q_->max_time_));
                }
                iters.push_back(mit);
            }
            ...
            for (int i = 0; i < q_->imms_.size(); i++) {
                iters.push_back(new MemtableIterator(q_->value_log_, q_->imms_[i], q_->min_time_, q_->max_time_, series_id_));
            }
            if (q_->mem_) {
                iters.push_back(new MemtableIterator(q_->value_log_, q_->mem_, q_->min_time_, q_->max_time_, series_id_));
            }
        }
//        bool flag = 0;
        if (q_->max_time_ > level_flush_time_) {
            // Add ValueLog iterator
            iters.push_back(new ValueLogIterator(q_->value_log_, std::max(q_->min_time_,level_flush_time_), q_->max_time_, series_id_));
        }
    ...
    std::unique_ptr<querier::SeriesIteratorInterface> SpanQuerierSeries::chain_iterator() {
        if (!init_) {
            q_->head_->series(series_id_, lset_, &head_chunk_contents_);
            init_ = true;
        }
        ...
        if (q_->min_time_ <= level_flush_time_) {
            ...
            for (int i = 0; i < q_->l0_indexes_.size(); i++) {
                std::vector<leveldb::Iterator*> list;
                q_->current_->AddIterators(leveldb::ReadOptions(), 0, q_->l0_indexes_[i], &list, series_id_);
                for (leveldb::Iterator* it : list) {
                    iters.push_back(new L0ValueLogIterator(q_, series_id_, it, q_->min_time_, q_->max_time_));
                }
            }
...
//        bool flag = 0;
        if (q_->max_time_ > level_flush_time_) {
            // Add ValueLog iterator
            iters.push_back(new ValueLogIterator(q_->value_log_, std::max(level_flush_time_,q_->min_time_), q_->max_time_, series_id_));
        }
    ...
    /* ****************************************
     *  SpanQuerierSeriesSet
     * **************************************** */

    SpanQuerierSeriesSet::SpanQuerierSeriesSet(const tsdb::querier::SpanQuerier *q, const std::vector<std::shared_ptr<label::MatcherInterface>> &l)
            : q_(q),
              matchers_(l),
              series_id_(0) {
        auto p = q->head_->select(l);
        if (!p) {
            err_.set("error SpanQuerierSeriesSet postings for matchers");
            LOG_DEBUG << "error SpanQuerierSeriesSet postings for matchers";
            p_.reset();
        } else {
            p_  = std::move(p);
        }
    ...
    bool SpanQuerierSeriesSet::next() const {
        if (err_) return false;
    ...
    std::unique_ptr<querier::SeriesInterface> SpanQuerierSeriesSet::at() {
        if (err_) return nullptr;
        if (series_id_ == 0) return nullptr;
        if (q_->head_->read_flat_forward_index(series_id_) == nullptr) return nullptr;
        return std::unique_ptr<querier::SeriesInterface>(new SpanQuerierSeries(q_, series_id_));
    }
    ...
    void SpanQuerier::register_mem_partitions() {
        mem_ = db_->mem();
        imms_ = *db_->imms();
    }

    void SpanQuerier::register_disk_partitions() {
    ...
    std::unique_ptr<::tsdb::querier::SeriesSetInterface> SpanQuerier::select(std::vector<std::shared_ptr<::tsdb::label::MatcherInterface>> &l) const {
        return std::unique_ptr<::tsdb::querier::SeriesSetInterface>(new SpanQuerierSeriesSet(this, l));
    }
