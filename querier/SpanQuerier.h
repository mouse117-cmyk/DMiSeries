...
namespace tsdb {
    ...
    namespace querier {
        ...
        public:
            SlabArrayIterator(slab::ValueLog* value_log, std::vector<std::pair<const slab::SlabInfo*, slab::Slab*>> slab_array, int64_t min_time, int64_t max_time, uint64_t series_id, leveldb::Cache* cache = nullptr);
            ~SlabArrayIterator();
            ...
            void DecodeItemChunkHeader(slab::Item* item, uint16_t& mid, uint64_t& series_id, int64_t& start_time) const;
        };
        ...
        public:
            ValueLogIterator(slab::ValueLog* value_log, int64_t min_time, int64_t max_time, uint64_t series_id, leveldb::Cache* cache = nullptr);
            ~ValueLogIterator();
            bool seek(int64_t t) const override;
            std::pair<int64_t, double> at() const override;
            bool next() const override;
        ...
        public:
            MemtableIterator(slab::ValueLog* value_log, leveldb::MemTable* mem, int64_t min_time, int64_t max_time, uint64_t series_id, leveldb::Cache* cache = nullptr);
            ~MemtableIterator();
        ...
        public:
            L0ValueLogIterator(const querier::SpanQuerier* q, int partition, uint64_t series_id, int64_t mint, int64_t maxt);
            L0ValueLogIterator(const querier::SpanQuerier* q, uint64_t series_id, leveldb::Iterator* it, int64_t mint, int64_t maxt);
            ~L0ValueLogIterator();
        ...
        public:
            L1ValueLogIterator(const querier::SpanQuerier* q, int partition, uint64_t series_id, int64_t mint, int64_t maxt);
            ~L1ValueLogIterator();
        ...
        public:
            SpanQuerierSeries(const SpanQuerier* q, uint64_t series_id);
            ...
            uint32_t get_sid(slab::ValueLog* span_series, head::SpanHead* head, uint64_t series_id, uint16_t mid) {
                auto tms = head->read_flat_forward_index(series_id);
                return tms->sid_;
            }

            std::vector<std::pair<const slab::SlabInfo*, slab::Slab*>> get_head_slab_array(slab::ValueLog* value_log, head::SpanHead* head, uint64_t series_id) {
                auto tms = head->read_flat_forward_index(series_id);
                ...
                for(uint8_t i=0;i<sinfo->offset_;i++){
                    if(series_id == value_log->GetSinfoSourceID(sinfo,i)){
                        new_sinfo->start_time_[0] = sinfo->start_time_[i];
                        break;
                    }
                }
                new_sinfo->series_id_[0] = series_id / slab::CY_NUM;
                new_sinfo->offset_ = 1;
        ...
        public:
            ...
            uint64_t  current_sgid() override { return series_id_; }
            bool error() const override {return err_;}
        };
