...
void PutFixed16BE(std::string* dst, uint16_t value) {
    char buf[sizeof(value)];
    EncodeFixed16BE(buf, value);
    dst->append(buf, sizeof(buf));
}
...
//    leveldb::PutFixed64(key, sgid);
//    leveldb::PutFixed16(key, mid);
//    leveldb::PutFixed64(key, end_time);
//}
//
//void decode_l1key(std::string key, uint64_t &sgid, uint16_t &mid, uint64_t &end_time) {
//    sgid = leveldb::DecodeFixed64(key.data());
//    mid = leveldb::DecodeFixed16(key.data()+8);
//    end_time = leveldb::DecodeFixed64(key.data()+10);
//}
