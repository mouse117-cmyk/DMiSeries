...
inline void EncodeFixed16BE(char* buf, uint16_t value) {
    buf[0] = (value >> 8) & 0xff;
    buf[1] = value & 0xff;
}
...
inline uint16_t DecodeFixed16BE(const char* ptr) {
    return ((static_cast<uint16_t>(static_cast<unsigned char>(ptr[0])) << 8) |
            (static_cast<uint16_t>(static_cast<unsigned char>(ptr[1]))));
}
