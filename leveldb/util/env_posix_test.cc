...
// Exit codes for the helper process spawned by TestCloseOnExec* test.
...
// Executed in a separate process by TestCloseOnExec* test.
//
// main() delegates to this function when the test executable is launched with
// a special command-line switch. TestCloseOnExec* test fork()+exec() the test
...
int main(int argc, char** argv) {
  ...
  // All test currently run with the same read-only file limits.
