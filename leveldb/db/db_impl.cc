...
Status DBImpl::WriteLevel0TableSpan(MemTable* mem, VersionEdit* edit,
                                    Version* base) {
    mutex_.AssertHeld();
    const uint64_t start_micros = env_->NowMicros();
    // FileMetaData meta;
    ...
    Status s;
    {
        ...
        // s = BuildTable(dbname_, env_, options_, table_cache_, iter, &meta,
        // head_);
        s = BuildTablesSpan(dbname_, env_, options_, table_cache_, iter, &metas,
                            versions_, &pending_outputs_, PARTITION_LENGTH, span_head_);
        mutex_.Lock();
    }
...
void DBImpl::CompactMemTableSpan() {
    mutex_.AssertHeld();
    // assert(imm_ != nullptr);
    assert(!imms_.empty());
    MemTable* imm = imms_[0];
    ...
    Version* base = versions_->current();
    base->Ref();
    Status s = WriteLevel0TableSpan(imm, &edit, base);
    base->Unref();
...
void DBImpl::BackgroundCall() {
  ...
  if (shutting_down_.load(std::memory_order_acquire)) {
    ...
    // No more background work after a background error.
      ...
      if (span_head_ != nullptr) {
          BackgroundCompactionSpan();
      } else if(tree_head_!= nullptr) {
...
void DBImpl::BackgroundCompactionSpan() {
    mutex_.AssertHeld();
    ...
    if (c == nullptr) {
        ...
        CompactionState* compact = new CompactionState(c);
        status = DoCompactionWorkSpan(compact);
        if (!status.ok()) {
            RecordBackgroundError(status);
        }
        CleanupCompaction(compact);
...
Status DBImpl::DoCompactionWorkSpan(CompactionState* compact) {
    const uint64_t start_micros = env_->NowMicros();
    int64_t imm_micros = 0;  // Micros spent doing imm_ compactions
...
//        DeCodeKey(ikey.user_key.ToString(), current_sgid, current_mid, current_tuple_time);
        slab::decode_l1key(ikey.user_key.ToString(), current_sgid, current_mid, current_tuple_time);
