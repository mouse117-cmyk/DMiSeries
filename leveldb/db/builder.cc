...
Status BuildTablesSpan(const std::string& dbname, Env* env, const Options& options,
                       TableCache* table_cache, Iterator* iter,
                       std::vector<FileMetaData>* metas, VersionSet* const vset,
                       std::set<uint64_t>* pending_outputs,
                       int64_t partition_length, ::tsdb::head::SpanHead* head) {
    Status s;
    iter->SeekToFirst();
    ...
    if (iter->Valid()) {
        ...
        for (; iter->Valid(); iter->Next()) {
            ...
            user_key = ExtractUserKey(key);
//            DeCodeKey(user_key.ToString(), sgid, mid, key_time);
            slab::decode_l1key(user_key.ToString(), sgid, mid, key_time);
            time_boundary = key_time / partition_length * partition_length;
